...
\begin{titlepage}
    ...
    \textbf{\textsc{\large Superior School of Computer Sciences (ESCOM) \\ Instituto Politécnico Nacional (IPN)}}                          %Name of project 
    \vspace{2.7cm}                                                  %Name of project   

    % ============ NAME OF THE BOOK  ===============
    \href{ProjectNameLink/Book}                                     %Link to Author
    {\fontsize{45}{48}\selectfont \textbf{How long would it take to break AES?}}\\[0.5cm]     %Name of the book
    \textcolor{ColorSubtext}{\textsc{\Huge Partial 2}}          %Name of the general theme
...
\begin{itemize}
    \item We have a butget of $1 000 000$ dollars.
    \item Each processor cost $10$ dollars.
    \item So, we can have up to $100 000$ processors.
    \item Each processor take $t$ time to check a key. (let $t = 10^{-9}$ seconds)
    \item So all of them will take $\frac{t}{100 000}$ to check a key.
\end{itemize}
...
\begin{MultiLineEquation*}{3}
    num\_days = 2^{128} \times \frac{t}{100 000} \times \frac{1}{60 * 24}
\end{MultiLineEquation*}

Now, Moore's law says that each 18 months we double we performance, so:

\begin{MultiLineEquation*}{3}
    num\_days = 2^{128} (0.5)^{times} \times \frac{t}{100 000} \times \frac{1}{60 * 24}
\end{MultiLineEquation*}
...
\begin{MultiLineEquation*}{3}
    times = log_{0.5}\Wrap{
        \dfrac{7}{2^{128} \times \frac{t}{100 000} \times \frac{1}{60 * 24}}
    }
\end{MultiLineEquation*}
